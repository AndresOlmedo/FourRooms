...
public class MaskSilla : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, maskPosition) < 2.0f)
        {
            //Y se pulsa el botón F.
            if (Input.GetKeyDown(KeyCode.F))
            {
                //Se destruye la máscara. Se mueve el Vector3 para que la consola no intente buscar una posición que ya no existe. 
